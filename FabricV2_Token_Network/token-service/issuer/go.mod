...
require (
	...
	github.com/vikash-ftw/assetchain/token-service/views v0.0.0
)
...
require (
	...
	github.com/hyperledger-labs/fabric-token-sdk v0.10.1 // indirect
...
// The views module is local and unpublished - its import path is not a
// real repository. go.work's `use` directive covers `go build`, but
// `go mod tidy` resolves requires per-module and would try to fetch this
// from github. The replace keeps each module independently tidy-able.
// Path is relative to this go.mod, and holds inside the Docker build too
// (issuer/ and views/ are siblings under /workspace).
replace github.com/vikash-ftw/assetchain/token-service/views => ../views
